...
def solve_lp(G):
    """
    Solves the LP relaxation of vertex cover.
    Returns (objective_value, variable_dict)
    """
    ...
    return pulp.value(prob.objective), {v: x[v].value() for v in G.V}
...
def branch_and_bound(G, Z):
    global UB, lp_counter

    # If no edges remain, we have a vertex cover
    if not G.E:
        UB = min(UB, Z)
        return
